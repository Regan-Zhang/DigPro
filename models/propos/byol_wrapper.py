...
from lightly.models import utils as liutils
...
class BYOLWrapper(MoCoWrapper):
    ...
    def __init__(self,
        ...
        prototype_number = int(self.num_cluster)
        print(f"Prototype Number is {prototype_number}")
        self.smog = SMoGPrototypes(group_features=torch.rand(prototype_number, self.fea_dim), beta=0.99)
        self.memory_bank = MemoryBankModule(size=2560)
        # print(f"Memory bank size is {self.memory_bank.size}")
        # create the encoders
    ...
    def _cluster_features(self, features):
        ...
        kmeans = KMeans(self.num_cluster).fit(features)
        clusterd = torch.from_numpy(kmeans.cluster_centers_).float()
        clusterd = torch.nn.functional.normalize(clusterd, dim=1)
        return clusterd
    ...
    def reset_momentum_weights(self):
        # self.encoder_k = copy.deepcopy(self.encoder_q)
        # self.projector_k = copy.deepcopy(self.projector_q)
        liutils.deactivate_requires_grad(self.encoder_k)
        liutils.deactivate_requires_grad(self.projector_k)
    ...
    def forward_loss(self, im_q, im_k, psedo_labels: torch.Tensor):
        ...
        # SMog
        assignments = self.smog.assign_groups(k)
        group_features = self.smog.get_updated_group_features(q)
        logits = self.smog(self.predictor(q), group_features, temperature=0.1)
        self.smog.set_group_features(group_features)
        loss_fn = nn.CrossEntropyLoss()
        smog_loss = loss_fn(logits, assignments)
        ...
        return contrastive_loss, cluster_loss_batch, all_q, all_k, smog_loss

    def forward(self, im_q, im_k, indices, n_iter, momentum_update=True):
        """
        Input:
        ...
        Output:
            logits, targets
        """
        if n_iter > 1 and n_iter % self.num_cluster == 0:
        # # if n_iter > 1 and n_iter % 4 == 0:  # For AID D0-40
        # # if n_iter > 1 and n_iter % 10 == 0:  # For DTD47
        # # if n_iter > 1 and n_iter % 20 == 0:  # For Tiny-ImageNet
            self.reset_group_features(memory_bank=self.memory_bank)
            self.reset_momentum_weights()
        else:
            liutils.update_momentum(self.encoder_q, self.encoder_k, 0.99)
            liutils.update_momentum(self.projector_q, self.projector_k, 0.99)

        psedo_labels = self.psedo_labels[indices]
        if self.symmetric:
            contrastive_loss1, cluster_loss_batch1, q1, k1, smog_loss_1 = self.forward_loss(im_q, im_k, psedo_labels)
            contrastive_loss2, cluster_loss_batch2, q2, k2, smog_loss_2 = self.forward_loss(im_k, im_q, psedo_labels)

            smog_loss = 0.5 * (smog_loss_1 + smog_loss_2)
            ...
            # contrastive_loss, cluster_loss_batch, q, k, dcl_loss = self.forward_loss(im_q, im_k, psedo_labels)
            contrastive_loss, cluster_loss_batch, q, k, smog_loss = self.forward_loss(im_q, im_k, psedo_labels)

        # contrastive_loss, cluster_loss_batch, q, k, smog_loss = self.forward_loss(im_q, im_k, psedo_labels)

        # memory bank update
        self.memory_bank(q, update=True)
        # print(f"memory_bank {self.memory_bank.bank.size()}")
        ...
        return contrastive_loss, cluster_loss_batch, q, smog_loss
