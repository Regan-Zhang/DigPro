...
class BYOL(TrainTask):
    ...
    def train(self, inputs, indices, n_iter):
        ...
        with torch.autocast('cuda', enabled=opt.amp):
            contrastive_loss, cluster_loss_batch, q, smog_loss = self.byol(
                im_q, im_k, indices, n_iter, update_params)

        loss = contrastive_loss + cluster_loss_batch * opt.cluster_loss_weight * float(not is_warmup) + smog_loss*0.3
        # loss = contrastive_loss + smog_loss*0.1

        with open(f"{self.logger.log_save_dir}/loss_logger.txt", "a") as f:
            f.write(
                f"Epoch[{self.cur_epoch}] smog_loss={smog_loss}, contrastive_loss={contrastive_loss}, cluster_loss_batch={cluster_loss_batch}\n")
